...
impl Connection {
    ...
    /// The peer's advertised `initial_max_streams_bidi` transport parameter.
    ///
    /// This is the total number of client-initiated bidirectional streams the
    /// peer will allow us to open before it must send a `MAX_STREAMS` frame to
    /// raise the limit. Useful for diagnosing credit-starvation hangs on
    /// `open_bi()` where the peer advertises a low initial limit and never
    /// raises it at runtime.
    pub fn peer_params_initial_max_streams_bidi(&self) -> u64 {
        self.peer_params.initial_max_streams_bidi.into_inner()
    }

    /// The peer's advertised `initial_max_streams_uni` transport parameter.
    pub fn peer_params_initial_max_streams_uni(&self) -> u64 {
        self.peer_params.initial_max_streams_uni.into_inner()
    }

    /// The peer's advertised `initial_max_data` transport parameter.
    pub fn peer_params_initial_max_data(&self) -> u64 {
